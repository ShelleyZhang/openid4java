...
public class YadisResolverTest extends TestCase
{
	...
	public YadisResolverTest(final String testName) throws Exception
	{
		...
		_servletPort = Integer.parseInt(System.getProperty("SERVLET_PORT", "8989"));
	}
	...
	{
		YadisResult result =
			_resolver.discover("http://localhost:" + _servletPort + "/?headers=simpleheaders", 10,
	...
	{
		YadisResult result =
			_resolver.discover("http://localhost:" + _servletPort + "/?headers=simpleheaders_xml",
	...
	{
		YadisResult result =
			_resolver.discover("http://localhost:" + _servletPort + "/?html=simplehtml", 10,
				Collections.singleton("http://example.com/"));
	...
	public void testRedirectToHeaderResponse() throws DiscoveryException
	{
		YadisResult result =
			_resolver.discover(
				"http://localhost:" + _servletPort + "/?headers=redir_simpleheaders", 10,
				Collections.singleton("http://example.com/"));
	...
	{
		YadisResult result =
			_resolver.discover("http://localhost:" + _servletPort + "/?headers=redir_simplehtml",
				10, Collections.singleton("http://example.com/"));
	...
	{
		YadisResult result =
			_resolver.discover("http://localhost:" + _servletPort + "/?headers=redir_simplexrds",
				10, Collections.singleton("http://example.com/"));
	...
	public void testIncompleteHtmlParsing() throws DiscoveryException
	{
		...
		YadisResult result =
			resolver.discover("http://localhost:" + _servletPort + "/?html=simplehtml", 10,
				Collections.singleton("http://example.com/"));
	...
	/*
	 * public void testHtmlHeadElementsNoHead() { try {
	 * _resolver.discover("http://localhost:" + _servletPort + "/?html=nohead");
	 * fail("Should have failed with error code " +
	 * OpenIDException.YADIS_HTMLMETA_INVALID_RESPONSE); } catch (DiscoveryException
	 * expected) { assertEquals(expected.getMessage(),
	 * OpenIDException.YADIS_HTMLMETA_INVALID_RESPONSE, expected.getErrorCode()); } }
	 * public void testHtmlHeadElementsTwoHeads() { try {
	 * _resolver.discover("http://localhost:" + _servletPort + "/?html=twoheads");
	 * fail("Should have failed with error code " +
	 * OpenIDException.YADIS_HTMLMETA_INVALID_RESPONSE); } catch (DiscoveryException
	 * expected) { assertEquals(expected.getMessage(),
	 * OpenIDException.YADIS_HTMLMETA_INVALID_RESPONSE, expected.getErrorCode()); } }
	...
	public void testHtmlHeadElementsExtraHeadInBody()
	{
		...
		{
			YadisResult result =
				_resolver.discover("http://localhost:" + _servletPort + "/?html=extraheadinbody",
					10, Collections.singleton("http://example.com/"));
	...
	public void testHtmlHeadNoMeta() throws DiscoveryException
	{
		...
		assertEquals("Should have discovered no endpoints; found: " + result.size(), result.size(),
			0);
	}

	/*
	 * public void testEmptyHtml() { try { _resolver.discover("http://localhost:" +
	 * _servletPort + "/?html=empty"); fail("Should have failed with error code " +
	 * OpenIDException.YADIS_HTMLMETA_INVALID_RESPONSE); } catch (DiscoveryException
	 * expected) { assertEquals(expected.getMessage(),
	 * OpenIDException.YADIS_HTMLMETA_INVALID_RESPONSE, expected.getErrorCode()); } }
	 */
	...
	public void testGetError() throws Exception
	{
		...
		{
			assertEquals(expected.getMessage(), OpenIDException.YADIS_GET_ERROR,
				expected.getErrorCode());
		}
	}
	...
	public void testXrdsOpenidDelegate() throws Exception
	{
		...
		try
		{
			...
			assertNotNull("Should have discovered an openid:Delegate.",
	...
	public void testEmptyUri() throws Exception
	{
		...
		assertTrue("XRDS with an empty URI is valid; Yadis should have succeeded", yadis
			.getEndpoints().size() > 0);
		...
		List results =
			manager.discover("http://localhost:" + _servletPort
